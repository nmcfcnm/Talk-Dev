...
userRedirection=(req,res,user)=>{
    if(user.role=='admin' && user.adminAsUser==false) return res.redirect('/Admin/Profile');
    return res.redirect('/community/communitypanel');  }
function checkUser(req,res,user){
    if(user.firstTimeLogin==true) return res.redirect('/firstTimeLogin');
    if(req.session.redirectPathExist) return res.redirect(req.session.redirectPath);
...
app.get('/firstTimeLogin',redirectLogin,(req,res)=>{
    User.findById(req.session.userInfo._id).select("name image role dob gender email phone city interest journey expectation").exec(function(err, user){
    if (err) throw err;
    if(req.session.userInfo.firstTimeLogin==true) return res.render('firstTimeLogin',{user: user});
...
app.get('/',redirectHome,(req, res) =>{ res.render('login'); });
app.post('/',redirectHome,(req, res) =>{
    User.findOne({ email: req.body.email }).select("name image role activated adminAsUser firstTimeLogin email password").exec(function(err, user){
        if (err) throw err;
        if(user===null || user.password!==req.body.password){
            res.locals.isAuthFailed=true; return res.render('login'); }
        if(user.activated==false)  return res.render("userDeactivatedAccountAccess");
...
app.get('/admin/adduser',redirectLogin,(req,res)=>{ res.render('admin/addUser',{ user:req.session.userInfo }); });
app.post('/admin/adduser',redirectLogin,(req,res)=>{
    ...
    if (err) throw err;
    mkdirp('public/user_images/'+newUser._id,function(err){ return err; });
...
app.get('/Admin/Profile',redirectLogin,(req, res) =>{
        ...
        User.findById(req.session.userInfo._id).select("name city role adminAsUser gender email phone dob city interest journey expectation").exec(function(err, user){
        if (err) throw err;
...
app.post('/editProfile',redirectLogin,(req,res)=>{
    var _id=req.session.userInfo._id,name=req.body.name,dob=req.body.dob,gender=req.body.gender,phone = req.body.phone,city=req.body.city , expectation=req.body.expectation , journey=req.body.journey;
    User.updateOne({_id : _id},{ name: name ,  dob: dob , gender : gender , phone:phone , city : city , expectation:expectation , journey:journey}, function(err, user) {
    if (err) throw err;
...
app.post('/entries',redirectLogin,(req,res)=>{
    User.find().select("email phone city status role").exec(function(err, users){
...
app.post('/commentries',redirectLogin,(req,res)=>{
    Community.find().select('name communityType owner createdOn image info').exec(function(err, comm){
...
app.post('/changePassword',redirectLogin,(req,res)=>{
    ...
    User.updateOne({_id : req.session.userInfo._id},{ password: req.body.newPassword}, function(err, user) {
        if (err) throw err;
...
app.post('/community/AddCommunity',redirectLogin,(req, res) =>{
    ...
    comm.save(function(err) {
        if(err) throw err;
        User.updateOne({ _id: req.session.userInfo._id },
            { $push: { myCreatedCommunities: comm._id } }, function (err) {
                if (err) throw err;
            });
       res.render('user/createCommunity',{ user : req.session.userInfo });
    });
...
app.get('/community/communitypanel',redirectLogin,function(req,res){
    User.findById(req.session.userInfo._id).populate('myCreatedCommunities', '_id name image membersRequested').populate('adminCommunities', '_id name image membersJoined admin').populate('JoinedCommunties', '_id name image membersJoined admin').populate('RequestedCommunties', '_id name image membersJoined admin').populate('invited','_id name image membersJoined admin').select('myCreatedCommunities adminCommunities JoinedCommunties RequestedCommunties invited').lean().exec(function (err, result) {
        if (err) throw err;
        return res.render('community/communitypanel', {user : req.session.userInfo , commPopulated:result});
    });
});
...
app.get('/community/list',redirectLogin,(req,res)=>{
    Community.find( { $and: [ {  membersJoined : { $nin : [req.session.userInfo._id]}    } , {  owner : { $nin : [req.session.userInfo._id]}    }  , {  admin : { $nin : [req.session.userInfo._id]}    }  , {  invited : { $nin : [req.session.userInfo._id]}    }  , {  membersRequested : { $nin : [req.session.userInfo._id]}    }  ] }).select('_id name communityType info name image').exec(function(err,comm){
        res.render('community/list',{ user:req.session.userInfo, c : comm });        
    });
});
app.post('/joinCommunityRequest',redirectLogin,(req, res) =>{
    var userid=req.session.userInfo._id,commid=req.body.id;
    Community.findOne({_id : commid }).select('communityType').exec(function(err,comm){
    if(err) throw err;
...
app.get('/community/communityprofile/:_id',redirectLogin,(req, res) =>{
    ...
    Community.findOne({_id : req.params._id }).populate('membersJoined', '_id name image').populate('admin', '_id name image').populate('membersRequested', '_id name image').populate('RequestedCommunties', '_id name image').populate('owner', '_id name image').lean().exec(function (err,comm) {
        if (err) throw err;
        ...
        if(checkType(comm.membersJoined,id)){ communityInteraction='joined'; }
        if(checkType(comm.admin,id)){ communityInteraction='admin'; }
        if(checkType(comm.invited,id)){ communityInteraction='invited'; }
        if(checkType(comm.membersRequested,id)){ communityInteraction='requested'; }
...
app.get('/community/discussion/:_id',redirectLogin,(req, res) =>{
        Community.findOne({_id : req.params._id }).select("name _id image").exec(function(err,community){
            if(err) throw err;
...
app.get('/community/editcommunity/:_id',redirectLogin,(req,res)=>{
    Community.findOne({_id : req.params._id }).select("name _id communityType info image").exec(function(err,community){
        if(err) throw err;
...
app.post('/community/editcommunity/:_id',redirectLogin,(req,res)=>{
    var _id=req.params._id,name=req.body.name,communityType=req.body.communityType,info=req.body.communityDescription,image = req.body.image;
    Community.updateOne({_id : _id},{ name: name ,  communityType:communityType , info : info , image:image}, function(err, user) {
    if (err) throw err;
...
app.get('/community/managecommunity/:_id',redirectLogin,(req, res) =>{
    Community.findOne({_id : req.params._id }).select("name _id image membersJoined membersRequested invited admin").exec(function(err,community){
        if(err) throw err;
...
app.post('/community/allcomadminsandowners/:_id',redirectLogin,(req,res)=>{
    Community.findOne({_id : req.params._id }).populate('admin', '_id name image').populate('owner', '_id name image').select('owner admin').lean().exec(function (err,comm) {
        if (err) throw err;
...
app.post('/community/membersRequested/:_id',redirectLogin,(req,res)=>{
    Community.findOne({_id : req.params._id }).populate('membersRequested', '_id name image').select('membersRequested').lean().exec(function (err,comm) {
        if (err) throw err;
        return res.send(JSON.stringify({membersRequested : comm.membersRequested}));
    });
});
app.post('/community/invited/:_id',redirectLogin,(req,res)=>{
    Community.findOne({_id : req.params._id }).populate('invited', '_id name image').select('invited').lean().exec(function (err,comm) {
        if (err) throw err;
...
app.post('/community/invite/:_id',redirectLogin,(req,res)=>{
    var userid = req.session.userInfo._id,commid=req.params._id;
    ...
    var query = { $and: [search, {  JoinedCommunties : { $nin : [commid]}    } , {  myCreatedCommunities : { $nin : [commid]}    }  , {  admin : { $nin : [commid]}    }  , {  invited : { $nin : [commid]}    }  , {  RequestedCommunties : { $nin : [commid]}    }] };
    User.find(query).select('_id name image').lean().exec(function (err, result) {
        if (err) throw err;
...
app.post('/listPost/:_id',(req, res) =>{
    Post.find({ communityId: req.body.commid }).populate('createdById','name').lean().exec(function (err, post) {
    if (err) throw err;
...
app.post('/createPost/:_id',redirectLogin,(req, res) =>{
    ...
    post.save(function(err) {
        ...
        Comment.count({postId: post._id}, function(err, c){
                res.send(JSON.stringify({ post : post , createdById : req.session.userInfo._id , createdByName : req.session.userInfo.name ,commentsCount : c}));
           });
    });
});
...
app.post('/GlobalThePost',redirectLogin,(req,res)=>{
    Post.findOne({_id: req.body.postid }, function (err, post) {
        ...
        post.save(function (err) {
            if(err) throw err;
            return res.send(JSON.stringify({_id:req.body.postid,global : post.global}));
        });
    });
});
app.post('/featureThePost',redirectLogin,(req,res)=>{
    Post.findOne({_id: req.body.postid }, function (err, post) {
        ...
        post.save(function (err) {
            if(err) throw err;
            return res.send(JSON.stringify({_id:req.body.postid,featured : post.featured}));
        });
    });
});
app.post('/listGlobalPostsOnly',(req,res)=>{
    Post.find( { $and: [ { communityId: req.body.commid }, { global: true } ] }).populate('createdById','name').lean().exec(function(err,post){
        if(err) throw err;
        res.send(JSON.stringify(post));
    });
});
...
app.post('/showCommenting',redirectLogin,(req,res)=>{
    Comment.find({postId : req.body.postid}).populate("createdById","name image").lean().exec(function(err,comment){
        res.send(JSON.stringify({comment:comment,image : req.session.userInfo.image}));
    });
    
});

app.post('/postANewComment',redirectLogin,(req,res)=>{
    var postid=req.body.postid,commentBody=req.body.commentBody;
    comment = new Comment({
    ...
    comment.save(function(err) {
        if(err) throw err;
            return res.send(JSON.stringify({ comment : comment ,name : req.session.userInfo.name , image  : req.session.userInfo.image , userid : req.session.userInfo._id }));
    });
});
...
app.post('/postANewReply',redirectLogin,(req,res)=>{
    var comment=req.body.commentid,reply=req.body.replyBody;
    reply = new Reply({
        replyBody : reply,
        commentId : comment,
        createdById : req.session.userInfo._id
    ...
    reply.save(function(err) {
        if(err) throw err;
            return res.send(JSON.stringify({ reply : reply ,name : req.session.userInfo.name , image  : req.session.userInfo.image , userid : req.session.userInfo._id }));
    });
});
app.post('/showReplying',redirectLogin,(req,res)=>{
    Reply.find({commentId : req.body.commentid}).populate("createdById","name image").lean().exec(function(err,reply){
